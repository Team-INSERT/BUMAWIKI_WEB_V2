...
const Aside = ({ display }: AsidePropsType) => {
  ...
  const { refetch } = useQuery(
    [queryKey.getLastModify],
    onGetLastModifiedDocs,
    {
      onSuccess: (data) => {
    ...
    <S.AsideWrap display={display}>
      ...
      {lastModifiedDocs.map((docs: Docs) => (
        <S.AsideDocWrap key={docs.id}>
          <S.AsideList href={`/docs/${docs.title}`}>
            {utils.asideFormat(docs.title, docs.docsType)}
          </S.AsideList>
          <S.AsideLastModified>
            &nbsp; {utils.getLastDate(docs.lastModifiedAt)}
          </S.AsideLastModified>
        </S.AsideDocWrap>
      ))}
      <S.AsidePageWrap>
        ...
        <S.AsidePageButton
          onClick={() =>
            setPage(lastModifiedDocs.length < 12 ? page : page + 1)
          }
        >
