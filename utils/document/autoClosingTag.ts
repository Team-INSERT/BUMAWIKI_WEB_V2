const autoClosingTag = (
  ...
  if (value[selectionStart - 1] === ">") {
    ...
    if (
      !isWrongTag.includes("<") ||
      openingTag.includes("/") ||
      openingTag.includes("사진")
    )
      return value;

    if (openingTag.includes("링크")) {
      text +=
        `</${openingTag.substring(0, openingTag.indexOf(" "))}>` +
    ...
    const closingTag = textAfterSelection
      .slice(
        ...
        textAfterSelection.indexOf(">") + 1,
      )
      .replace("/", "");

    if (
      insideTag === closingTag ||
      (insideTag.includes("링크") && closingTag.includes("링크"))
    )
      return value;
