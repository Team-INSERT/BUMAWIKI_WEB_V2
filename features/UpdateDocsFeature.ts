...
const updateDocsForm = ({ contents, files }: UpdateDocsFormType) => {
  ...
  files
    .reverse()
    .forEach((file) => data.append("files", file.object, file.object.name));
  ...
  return useMutation(onUpdateDocs, {
    ...
    onError: (err) => {
      if (err instanceof AxiosError && err.response) {
        ...
        if (status === 403) {
          if (message === exception.code.DOCS_403_2)
            toast.error("자기 자신의 문서는 편집할 수 없습니다.");
          if (error === "Forbidden")
            toast.error("읽기전용 사용자는 문서를 편집할 수 없습니다.");
        }
      }
    },
  });
