...
const useLikeCountById = (docs?: Docs) => {
  ...
  const getLikeCounts = async () => {
    if (!!docs)
      return (await httpClient.getLike.getByTitle(docs.title)).data
        .thumbsUpsCount;
  };

  const getIsLike = async () => {
    if (!!docs)
      return (await httpClient.isLike.getByTitle(docs.id.toString())).data;
  };
  ...
  const deleteLike = () => {
    if (!!docs)
      return httpClient.deleteLike.delete({ data: { docsId: docs.id } });
  };
